...
// optional: default manager id not used anymore, kept for compat
...
router.post('/time-off', requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      `,
        ...
        const text = `${user.fullName || user.email} has submitted a time off request for ${summaryDates}\nReason: ${body.reason}`;
        const to = ['hr@republicfinancialservices.com', ...approverEmails];
        await sendEmail({
            from: 'noreply@republicfinancialservices.com',
            to,
            subject,
            text,
        }).catch(() => null);
